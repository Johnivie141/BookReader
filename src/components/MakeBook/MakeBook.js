...
class MakeBook extends Component{
  SpeechOnEnd(event){
    ...
    speechRecognitionStart=  myQuill.getContents();
      console.log("LISTENING STATE IS " + this.state.listening);
   ...
   /*
    ...
    if (this.props && this.props.general && this.props.general.postDetail)
      ...
      let mypost={
         ...
         post_title:this.refs.bookTitle.value,
  ...
  if (props && props.getPostDetail && props.general ) {
    ...
    if (props && props.general && props.general.postDetail && !this.state.loaded){
      console.log("LOADING " + props.general.postDetail.post_text);
       this.setState({loaded:true,text:props.general.postDetail.post_text,bookTitle:props.general.postDetail.post_title})
  }
...
toggleListening(){
 if (this.state.listening){
  console.log("CHANGE STATE TO  NOT LISTENING");
	 this.state.recognition.stop();
     this.setState({listening:false})

 }
 else{
     console.log("CHANGE STATE TO LISTENING");
...
<div className="CreateBlogMainImageContainer" style={imageStyle} onClick={this.handleFile}>
   ...
   <div className="bookTitleDiv" >
     <span className="bookTitleLabel">Title</span>
     <input type="text" defaultValue={this.state.bookTitle} ref="bookTitle" size="80"/>
   </div>
        ...
        <ReactQuill 
          ...
          modules={MakeBook.modules}
          formats={MakeBook.formats}
...
export default connect(mapStateToProps,{})(MakeBook);
